...
SELECT game_name, count(*) AS "games played" from card_games Group By game_name;
Select game_name, MAX (score) AS "high score" from card_games Group By game_name;
Select date_played, SUM(score) AS "total points" from card_games group by date_played having "total points" > 1000 ;

SELECT count(*) AS "matches played", CASE
    WHEN date_played LIKE "%/01/%"
        THEN "January"
    WHEN date_played LIKE "%/02/%"
        THEN "February"
    When date_played LIKE "%/03/%"
        THEN "March"
    Else "Other"
END as "month played"
from card_games GROUP BY "month played";

Select date_played from card_games WHERE game_name = "Rummy" OR "Crazy Eights";
